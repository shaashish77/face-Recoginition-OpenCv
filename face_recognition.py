...
class Face_Recognition:
    def __init__(self, root):
        ...
        # title
        title_lbl = Label(self.root, text="FACE RECOGNITION", font=("serif", 30, "bold"),
                          bg="white", fg="green")
        title_lbl.place(x=0, y=0, width=1530, height=45)

        img_left = Image.open(r"college_images/face_detector1.jpg")
        img_left = img_left.resize((650, 700), Image.ADAPTIVE)
        self.photoimg_left = ImageTk.PhotoImage(img_left)

        first_label = Label(self.root, image=self.photoimg_left)
        first_label.place(x=0, y=55, width=650, height=700)

        img_right = Image.open(r"college_images"
                               r"/facial_recognition_system_identification_digital_id_security_scanning_thinkstock_858236252_3x3-100740902-large.jpg")
        img_right = img_right.resize((950, 700), Image.ADAPTIVE)
        self.photoimg_right = ImageTk.PhotoImage(img_right)

        first_label = Label(self.root, image=self.photoimg_right)
        first_label.place(x=650, y=55, width=950, height=700)

        btn1_1 = Button(first_label, command=self.face_recog, text="Face Recognition", cursor="hand2",
                        font=("serif", 17, "bold"),
                        bg="darkgreen", fg="yellow")
        btn1_1.place(x=365, y=620, width=200, height=40)
        ...
        def draw_boundary(img, classifier, scalefactor, minneighbors, color, text, clf):
            ...
            for (x, y, w, h) in features:
                ...
                if confidence > 75:
                    ...
                    cv2.putText(img, "Unknown Individual", (x, y - 5), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255),
                                3)
